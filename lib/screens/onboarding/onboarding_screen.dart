...
class _OnboardingScreenState extends State<OnboardingScreen> {
  ...
  Widget _welcomePage() => Scaffold(
    body: EvermoreBackground(child: SafeArea(child: Padding(
  ...
  Widget _goalsPage() => Scaffold(
    ...
    body:EvermoreBackground(child:ListView(padding:const EdgeInsets.fromLTRB(20,10,20,30),children:[
      const _StepIndicator(step:1,total:3),const SizedBox(height:18),
      ...
      ...goals.map((goal){final isSelected=selected.contains(goal);return Padding(padding:const EdgeInsets.only(bottom:9),child:Material(color:Colors.transparent,child:InkWell(borderRadius:BorderRadius.circular(18),onTap:(){HapticFeedback.selectionClick();setState(()=>isSelected?selected.remove(goal):selected.add(goal));},child:Ink(padding:const EdgeInsets.all(15),decoration:BoxDecoration(color:isSelected?EvermoreTheme.primary.withValues(alpha:.075):Colors.white.withValues(alpha:.72),borderRadius:BorderRadius.circular(18),border:Border.all(color:isSelected?EvermoreTheme.primary:EvermoreTheme.divider),boxShadow:isSelected?EvermoreTheme.cardShadow:const []),child:Row(children:[Expanded(child:Text(goal,style:const TextStyle(fontWeight:FontWeight.w700,fontSize:12.5))),Icon(isSelected?Icons.check_circle_rounded:Icons.radio_button_unchecked_rounded,color:isSelected?EvermoreTheme.primary:EvermoreTheme.muted)]))))});
      const SizedBox(height:10),NeoPillButton(label:'Continue',icon:Icons.arrow_forward_rounded,onPressed:selected.isEmpty?null:()=>setState(()=>page=2)),
    ]))
  );

  Widget _profilePage(){
    final canFinish=nameController.text.trim().isNotEmpty&&!saving;
    return Scaffold(appBar:AppBar(title:const Text('Set up your profile'),leading:IconButton(icon:const Icon(Icons.arrow_back_rounded),onPressed:saving?null:()=>setState(()=>page=1))),body:EvermoreBackground(child:SafeArea(child:Padding(padding:const EdgeInsets.fromLTRB(20,10,20,24),child:Column(crossAxisAlignment:CrossAxisAlignment.start,children:[
      const _StepIndicator(step:2,total:3),const SizedBox(height:18),
      const Text("This is how you'll show up",style:TextStyle(fontSize:25,fontWeight:FontWeight.w800,letterSpacing:-.5)),const SizedBox(height:8),
      const Text("Add your name and, if you'd like, a photo. This stays on your device only.",style:TextStyle(color:EvermoreTheme.muted,height:1.45,fontSize:12.5)),const SizedBox(height:30),
      Center(child:AvatarPicker(photoPath:photoPath,name:nameController.text,size:108,onChanged:(path)=>setState(()=>photoPath=path))),const SizedBox(height:26),
      const Text('Your name',style:TextStyle(fontWeight:FontWeight.w700,fontSize:13)),const SizedBox(height:8),
      TextField(controller:nameController,textCapitalization:TextCapitalization.words,onChanged:(_)=>setState((){}),style:const TextStyle(fontWeight:FontWeight.w600),decoration:const InputDecoration(hintText:'Enter your name',prefixIcon:Icon(Icons.badge_outlined))),
      const Spacer(),NeoPillButton(label:saving?'Creating your path...':'Start Evermore',icon:Icons.arrow_forward_rounded,onPressed:canFinish?finish:null),
    ])))))
    );
  }
}
...
class _StepIndicator extends StatelessWidget{
  final int step,total;const _StepIndicator({required this.step,required this.total});
  @override Widget build(BuildContext context)=>Row(children:List.generate(total,(i)=>Expanded(child:Container(margin:EdgeInsets.only(right:i==total-1?0:6),height:4,decoration:BoxDecoration(color:i<=step?EvermoreTheme.primary:EvermoreTheme.primary.withValues(alpha:.10),borderRadius:BorderRadius.circular(2))))));
}
